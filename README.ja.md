...
- **接続中の Wi-Fi**: キャプチャ対象が無線インターフェースなら、SSID・BSSID・暗号化なしかどうかをヘッダに表示します。macOS 14 以降は SSID と BSSID が位置情報サービスの許可対象なので、許可のないプロセスでは名前の代わりに「非開示（位置情報の許可が必要）」と表示されます（接続の有無は正しく分かります）。
